...
class MasterData extends CI_Model {
	public function getcountrylist($searchTerm = Null) {
		$sql   = "SELECT * FROM L_CountryMst WHERE CountryName Like '%$searchTerm%' and CountryID>0 order by CountryName asc";
	...
	public function getstatelist($country_id, $searchTerm = null) {
		...
		$query = $this->db->query($sql,array($country_id,$searchTerm));
		if (!is_null($searchTerm)) {
			$results = array();
			if ($query->result()) {
	...
	public function add_state() {
		...
		$sql   = //"CALL sp_statelistsearch('','')";
		"SELECT StateName FROM L_StateMst WHERE StateName = ? AND CountryID = ?";
		$query = $this->db->query($sql, array($data['StateName'], $data['CountryID']));
	...
	public function update_city($id = null, $mypincode = null, $mycityname = null) {
		...
		if ($data['CityName'] == "")
			$data['CityName'] = $mycityname;
		if ($data['Pincode'] == "")
			$data['Pincode'] = $mypincode;
		if ($data['CountryID'] != "") {
	...
	public function getDefaultservice($SServiceID = null, $Default_Service = null) {
		if ($SServiceID != null && $Default_Service != null) {
			if ($Default_Service == "true")
				$Default_Service = 1;
			if ($Default_Service == "false")
				$Default_Service = 0;
			$this->db->where('SServiceID', $SServiceID);
			$this->db->update('Service_Sub', array(
	...
	public function gettelecomlist($searchTerm = Null) {
		$sql   = "SELECT t.ID, t.CircleID, t.CName, t.CType, t.Code, b.StateName as BasicState, s.StateName, d.DistrictName, sd.SubDistrictName, ct.CityName FROM Tele_TelecomCircle t LEFT JOIN L_StateMst b on t.Basic_StateID=b.StateID LEFT JOIN L_StateMst s on t.StateID=s.StateID LEFT JOIN L_DistrictMst d on t.DistrictID=d.DistrictID and s.StateID=d.StateID LEFT JOIN L_SubDistrictMst sd on t.SubDistrictID=sd.SubDistrictID and d.DistrictID=sd.DistrictID LEFT JOIN L_CityMst ct on t.CityID=ct.CityID and sd.SubDistrictID=ct.SubDistrictID where ID Like '%$searchTerm%' Order By t.CircleID,t.StateID,t.DistrictID,t.SubDistrictID";
		$query = $this->db->query($sql);
		return $query;
	}
		...
		if ($this->input->post('my_state') != "") {
			...
			if (!is_null($data['CName']) && $query->num_rows() == 0) {
				foreach ($state as $key => $value) {
					$data['StateID']       = $value;
					$data['DistrictID']    = $District[$key] ? $District[$key] : -1;
					$data['SubDistrictID'] = $SubDistrictID[$key] ? $SubDistrictID[$key] : -1;
					$data['CityID']        = $CityID[$key] ? $CityID[$key] : -1;
	...
	public function update_mobileseries($MSeriesNo = null, $MTechnology = null, $CircleID = null, $BrandID = null) {
		...
		if ($data['MSeriesNo'] != "" || $data['MTechnology'] != "" || $data['CircleID'] != "" || $data['BrandID'] != "") {
			if ($data['MSeriesNo'] == "")
				$data['MSeriesNo'] = $MSeriesNo;
			if ($data['MTechnology'] == "")
				$data['MTechnology'] = $MTechnology;
			if ($data['CircleID'] == "")
